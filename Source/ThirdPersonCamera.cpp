...
void ThirdPersonCamera::Update(float dt)
{
	...
	float magicVerticalAngle = World::GetInstance()->GetPlayer() && World::GetInstance()->GetPlayer()->HasReachedGoal() ?
							   20 :
							   12;

	mVerticalAngle = (degrees(acos(dot(B, j))) * (uphill ? -1 : 1)) + magicVerticalAngle;
